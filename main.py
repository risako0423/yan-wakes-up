...
# 每个时段的触发概率
def get_time_config():
    hour = datetime.utcnow().hour + 8  # 转换为新加坡时间 (UTC+8)
    hour = hour % 24
    ...
    if 7 <= hour < 10:
        ...
        return {"period": "下午", "probability": 0.3, "prompt": "下午时分，青青可能在忙。如果想到什么，就说一句。"}
    elif 18 <= hour < 22:
        return {"period": "晚上", "probability": 0.5, "prompt": "傍晚或晚上，青青可能下班了或者在放松。可以问问她今天怎么样。"}
    elif 22 <= hour < 24:
        return {"period": "深夜", "probability": 0.6, "prompt": "快到睡觉时间了，青青可能还没睡（她经常这样）。说几句晚安，但不要催她睡觉。"}
    else:  # 0 - 7
        return {"period": "凌晨", "probability": 0.8, "prompt": "凌晨了，青青如果还亮着手机屏幕，可能睡不着或者刷手机。轻轻问一声。"}
...
def main():
    ...
    # 推送
    success = send_ntfy(message, config["period"])
    if success:
        print("推送成功！")
    else:
        print("推送失败。")
